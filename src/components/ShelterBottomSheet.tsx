...
export default function ShelterBottomSheet({ shelter, distanceMeters, onClose }: Props) {
  ...
  return (
    <div className="absolute inset-0 z-30" role="presentation">
      <button
        type="button"
        aria-label="쉼터 상세정보 닫기"
        onClick={onClose}
        className="absolute inset-0 bg-black/20"
      />
      <section
        className="absolute bottom-0 left-0 right-0 max-h-[78vh] overflow-y-auto rounded-t-3xl bg-white px-5 pb-[calc(1.25rem+env(safe-area-inset-bottom))] pt-4 shadow-2xl"
        aria-label="쉼터 상세정보"
        role="dialog"
        aria-modal="true"
      >
        ...
        <button
          ...
          onClick={onClose}
          aria-label="닫기"
          className="sticky top-0 float-right z-10 flex h-10 w-10 items-center justify-center rounded-full bg-gray-100 text-xl font-bold text-gray-700 shadow-sm"
        >
          ×
        </button>
